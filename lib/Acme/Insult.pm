package Acme::Insult 1.0 {    # https://kk-advice.koyeb.app/api
    use v5.38;
    use HTTP::Tiny;
    ...
    sub insult (%args) {
        my $res = _insult(%args);
        $res->{error} ? () : $res->{insult};
    }
    sub _insult (%args) { _http( insult => %args ); }    # Debugging

    sub adjective ( $lang //= 'en' ) {
        my $res = _adjective($lang);
        $res->{error} ? () : $res->{insult};
    }
    sub _adjective ($lang) { _http( adjective => ( lang => $lang ) ); }    # Debugging
}
1;
__END__
...
Insults are generated by L<libInsult|https://gitlab.com/mattia.basaglia/LibInsult>.
